CREATE OR REPLACE package body BMS.k_sa_gen_xlsx
is
--
...
--
  ...
  is
    ...
    t_comment raw(32767) := utl_raw.cast_to_raw( 'Implementation by Anton Scheffer, k_sa_gen_xlsx18' );
  begin
...
--
  ...
  procedure query2sheet  -----with row and column define
    (p_col pls_integer ,
     p_row pls_integer ,
     p_sql clob--varchar2
    , p_column_headers boolean := true
    ...
    t_cur_row pls_integer;
    t_useXf boolean := g_useXf;
    t_row_count pls_integer:=0;
    v_loop_number number:=0;
  begin
    setUseXf( p_UseXf );
    if p_sheet is null
    then
    ...
    dbms_sql.parse( t_c, p_sql, dbms_sql.native );
    dbms_sql.describe_columns2( t_c, t_col_cnt, t_desc_tab );
    for c in nvl(p_col,1) .. t_col_cnt+ (case when p_col is not null then p_col-1 else 0 end)
    loop
      if p_column_headers
      then
        cell( c, nvl(p_row,1), t_desc_tab( c ).col_name, p_sheet => t_sheet );
      end if;
      case
        when t_desc_tab( c ).col_type in ( 2, 100, 101 )
        then
...
--
    ...
    t_cur_row := case when p_row is not null then p_row+1 when  p_column_headers then 2 else 1 end;
...
--
    ...
    --  dbms_output.put_line('p_row_end='||p_row_end||'t_r'||t_r||' t_bulk_size ='||t_bulk_size);
      exit when t_r != t_bulk_size;
      t_cur_row := t_cur_row + t_r;
    end loop;
        p_no_of_rec :=t_r;
--        p_no_of_rec :=v_loop_number;--t_r;
    dbms_sql.close_cursor( t_c );
